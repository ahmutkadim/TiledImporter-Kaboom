...
class TiledImporter {
     ...
     */
    addTileLayer(layerIndex, sourceImage , tilesetIndex, gameObject, optimizationMode){
                ...
                if (tile.id == tileFrame){
                    for (let prop of tile.properties) {
                        if (checkProperty(prop, "solid", "bool", false, this.k)) {
                            if (!optimColl){
                                comps.push(this.k.area({collisionIgnore: ["tileLayer"]}));
     ...
     * @returns {GameObj<any>} A game object which the layer is implemented to.
     */
    addAllLayers(gameObject, sourceImage, tilesetIndex, optimizationMode){
        let obj = gameObject || this.k.add([
            this.k.pos(0, 0)
        ]);
        ...
        for (let i in layerDatas) {
            ...
            if (val.type == this.layerTypes.OBJECT_LAYER) {
                ...
                obj = this.addTileLayer(i, sourceImage, tilesetIndex, obj, optimizationMode);
            }
        }
        return obj;
    }
...
function optimizeCollision(map, cellSizeX, cellSizeY, k, horizontal){
    ...
    for (let poly of polys){
        ...
        d.rect = new k.Rect(k.vec2(0), poly.width * cellSizeX, poly.height * cellSizeY)
        res.push(d)
    }
    return res
}
